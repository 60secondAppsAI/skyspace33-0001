        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalDestinations = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDestinations">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Destination</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="DestinationId" type="text" placeholder="Enter DestinationId" v-model="destinationToAdd.destinationId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="destinationToAdd.name"></base-input>
  <base-input label="Country" type="text" placeholder="Enter Country" v-model="destinationToAdd.country"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="destinations" :row-key="record => record.DestinationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DestinationPictureView :destinations="destinations" />
          </div>
...
import DestinationPictureView from './DestinationPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    DestinationPictureView
  },

  data() {
    return {
      modalDestinations: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'destinationId',
          ...
          sorter: true
          //sorter: (a, b) => a.destinationId - b.destinationId,
      ...
      sortBy: 'destinationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDestinationsTable() {
      ...
      let destinationsTableData = [];
      for (let i = 0; i < this.destinations.length; i++) {
        destinationsTableData.push({
          id: i,
          destinationId: this.destinations[i].destinationId,
          year: this.destinations[i].year,
          date: this.destinations[i].date,
          competitionId: this.destinations[i].competitionId,
          destinationId: this.destinations[i].destinationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-destinations',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    routingToDestinationDetail(id) {
      this.$router.push({ name: 'DestinationDetail', params: { destinationId: id.toString() }})
    },
    routingToCityDetail(id) {
      this.$router.push({ name: 'CityDetail', params: { cityId: id.toString() }})
    },
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDestinationsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalDestinations = false;

      const currentDate = new Date().getTime();
      this.destinationToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.destinationToAdd);
      console.log(jsonData);
      
      const res = await DestinationService.addDestination(jsonData);
