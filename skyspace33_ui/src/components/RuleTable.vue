        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRules = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRules">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Rule</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RuleId" type="text" placeholder="Enter RuleId" v-model="ruleToAdd.ruleId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="ruleToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="rules" :row-key="record => record.RuleId" :pagination="pagination"
...
import RulePictureView from './RulePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'ruleId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRulesTable() {
      ...
      for (let i = 0; i < this.rules.length; i++) {
        rulesTableData.push({
          ...
          year: this.rules[i].year,
          date: this.rules[i].date,
          competitionId: this.rules[i].competitionId,
          ruleId: this.rules[i].ruleId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-rules',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRulesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.ruleToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.ruleToAdd);
      console.log(jsonData);
      
      const res = await RuleService.addRule(jsonData);
