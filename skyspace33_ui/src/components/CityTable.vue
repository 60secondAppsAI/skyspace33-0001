        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Citys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalCitys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCitys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add City</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CityId" type="text" placeholder="Enter CityId" v-model="cityToAdd.cityId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="cityToAdd.name"></base-input>
  <base-input label="Country" type="text" placeholder="Enter Country" v-model="cityToAdd.country"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="citys" :row-key="record => record.CityId" :pagination="pagination"
...
import CityPictureView from './CityPictureView.vue';
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'cityId',
          ...
          sorter: true
          //sorter: (a, b) => a.cityId - b.cityId,
          //sorter: (a, b) => a.cityId.localeCompare(b.cityId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} citys`,
      },
      ...
      sortBy: 'cityId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCitysTable() {
      ...
      for (let i = 0; i < this.citys.length; i++) {
        citysTableData.push({
          id: i,
          cityId: this.citys[i].cityId,
          year: this.citys[i].year,
          date: this.citys[i].date,
          competitionId: this.citys[i].competitionId,
          cityId: this.citys[i].cityId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-citys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCitysChanged', this.searchQuery);
		//this.renderCitysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCitys = false;

      const currentDate = new Date().getTime();
      this.cityToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.cityToAdd);
      console.log(jsonData);
      
      const res = await CityService.addCity(jsonData);
